...
from flask import flash, Response, make_response
...
from FantasyFootball import app, login_required
...
def showLogin():
    ...
    return render_template('login.html', year=datetime.now().year,
        title='Log in | FantasyFootball', STATE=state)
...
def newTeam():
    ...
    if request.method == 'POST':
        ...
        return render_template('newTeam.html',
            title = 'Add Team | FantasyFootball',
            year = datetime.now().year)
...
def showTeam(team_id):
    ...
    if 'username' not in login_session:
        return render_template('publicteam.html', 
            title='Team | FantasyFootball',
            year=datetime.now().year, team=team, players=players)
    else:
...
def editTeam(team_id):
    ...
    if team.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               " authorized to edit this team. Please create your" \
               " personal team in order to edit.');history.back();}" \
    ...
    if request.method == 'POST':
        ...
        return render_template('editTeam.html',
            title='Edit Team | FantasyFootball',
...
def deleteTeam(team_id):
    ...
    if team.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
                " authorized to delete this team. Please create your" \
                " own team in order to delete.');history.back();}" \
                "</script><body onload='myFunction()''>"
    if request.method == 'POST':
        players = session.query(Player).all()
        for p in players:
        ...
        return render_template('deleteTeam.html',
                               title='Delete Team | FantasyFootball',
                               year=datetime.now().year, team=team)
...
def newPlayer(team_id):
    ...
    if team.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               " authorized to add player. Please create your own"\
               " team in order to add player.');history.back();}" \
               "</script><body onload='myFunction()''>"
    if request.method == 'POST':
        ...
        return render_template('newPlayer.html',
                               title='Add Player | FantasyFootball',
                               year=datetime.now().year, team_id=team_id)
...
def editPlayer(team_id, player_id):
    ...
    if team.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               " authorized to edit player. Please create your own" \
               " team in order to add/edit player.');history.back();}" \
               "</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        return render_template('editPlayer.html',
                               title='Edit Player | FantasyFootball',
                               year=datetime.now().year, team_id=team_id,
...
def deletePlayer(team_id, player_id):
    ...
    if team.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not" \
               " authorized to delete player. Please create your own " \
               "team in order to add/edit/delete player.');history.back();}" \
               "</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if player.snap_url:
            # remove snap from folder
        ...
        return render_template('deletePlayer.html',
                               title='Delete Player | FantasyFootball',
                               year=datetime.now().year, player=player)
